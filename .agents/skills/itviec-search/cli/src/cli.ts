...
const USAGE = `ITviec job search - danh sách việc làm CNTT công khai tại Việt Nam
...
SEARCH FLAGS
  ...
  --remote <mode>        remote | hybrid | onsite. Lọc theo hình thức làm việc.
  --page <n>             Trang, tính từ 1. Mặc định 1.
  --limit, -n <n>        Giới hạn số kết quả trả về.
  --format <fmt>         json (mặc định) | table | plain.

GHI CHÚ
  robots.txt của itviec.com cho phép truy cập /it-jobs/*. Vẫn nên giữ tần suất thấp.
  Phần lớn tin ẩn mức lương sau đăng nhập; khi đó trường salary là null.
`
  ...
  for (const row of rows as Array<Record<string, unknown>>) {
    process.stdout.write(
      [
        ...
        row.workMode ? `  hình thức: ${String(row.workMode)}` : null,
