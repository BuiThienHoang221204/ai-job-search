...
SEARCH FLAGS
  ...
  --remote <mode>        remote | hybrid | onsite. Nhận để đồng bộ giao diện với
                         các portal khác; TopCV không ghi hình thức làm việc trên
                         thẻ tìm kiếm nên cờ này không lọc được gì.
  --page <n>             Trang, tính từ 1. Mặc định 1.
  --limit, -n <n>        Giới hạn số kết quả trả về.
  --format <fmt>         json (mặc định) | table | plain.

GHI CHÚ
  robots.txt của topcv.vn chỉ chặn các đường dẫn CV cá nhân (/cv/, /xem-cv/...);
  /tim-viec-lam-* và /viec-lam/* không bị chặn. Vẫn nên giữ tần suất thấp.
  Tin ghi "Thoả thuận" được trả về salary = null.
  Lọc địa điểm chạy ở phía client: TopCV lọc bằng cityIds[] với mã số không
  công bố.
`
